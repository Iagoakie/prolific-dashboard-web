            ...
            {prolificTab === 'auto' && (
              <div className="prolific-tab-content auto-config-grid">
                ...
                <div style={{ 
                  ...
                  <a 
                    href={`javascript:void(function(){var DASH_URL='${typeof window !== 'undefined' ? window.location.origin : 'https://prolific-dashboard-web.vercel.app'}';try{var tk=null;for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);var v=localStorage.getItem(k);if(v&&v.indexOf('eyJ')===0&&v.length>100){tk=v;break}}if(!tk){var cookies=document.cookie.split(';');for(var j=0;j<cookies.length;j++){var c=cookies[j].trim();if(c.indexOf('eyJ')!==-1){var parts=c.split('=');tk=parts.slice(1).join('=');break}}}if(!tk){alert('Token não encontrado. Faça login no Prolific primeiro.');return}var uid=null;try{var payload=JSON.parse(atob(tk.split('.')[1]));uid=payload.externalUserId||payload.sub}catch(e){}if(!uid){alert('Não foi possível extrair o User ID do token.');return}var headers={'Authorization':'Bearer '+tk};var accountData=null;var csvData=null;fetch('https://internal-api.prolific.com/api/v1/users/'+uid+'/',{headers:headers}).then(function(r){return r.json()}).then(function(data){accountData=data;return fetch('https://internal-api.prolific.com/api/v1/submissions/export/?ordering=-started_at',{headers:headers})}).then(function(r){if(!r.ok)throw new Error('Erro ao exportar CSV');return r.text()}).then(function(csvText){csvData=csvText;var w=window.open(DASH_URL,'_blank');if(!w){alert('Popup bloqueado! Por favor, autorize popups para este site.');return}var handshakeReceived=false;var onMessage=function(e){if(e.data==='dashboard_ready'){handshakeReceived=true;w.postMessage({type:'prolific_import',account:accountData,csv:csvData},'*');window.removeEventListener('message',onMessage)}};window.addEventListener('message',onMessage);setTimeout(function(){if(!handshakeReceived){w.postMessage({type:'prolific_import',account:accountData,csv:csvData},'*');window.removeEventListener('message',onMessage)}},3000)}).catch(function(err){alert('Erro ao sincronizar: '+err.message)})}catch(e){alert('Erro: '+e.message)}}())`}
